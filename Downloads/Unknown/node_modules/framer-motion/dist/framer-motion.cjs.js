    ...
    MotionValue.prototype.getVelocity = function () {
        ...
        return this.canTrackVelocity
            ? // These casts could be avoided if parseFloat would be typed better
                easingLookup.velocityPerSecond(parseFloat(this.current) -
...
function correctBoxShadow(latest, _viewportBox, delta, treeScale) {
     ...
     * the two scales.
     */
    var averageScale = easingLookup.mix(xScale, yScale, 0.5);
    // Blur
...
function applyAxisTransforms(final, axis, transforms, _a) {
    ...
    var originPoint = easingLookup.mix(axis.min, axis.max, axisOrigin);
...
function removeAxisDelta(axis, translate, scale, origin, boxScale) {
    ...
    var originPoint = easingLookup.mix(axis.min, axis.max, origin) - translate;
...
var clampProgress = function (v) { return easingLookup.clamp(0, 1, v); };
/**
...
function isNear(value, target, maxDistance) {
    ...
    return easingLookup.distance(value, target) < maxDistance;
}
/**
 * Calculate the translate needed to be applied to source to get target
 */
function calcTranslate(source, target, origin) {
    var sourcePoint = easingLookup.mix(source.min, source.max, origin);
    var targetPoint = easingLookup.mix(target.min, target.max, origin);
...
function calcOrigin(source, target) {
    ...
    if (targetLength > sourceLength) {
        origin = easingLookup.progress(target.min, target.max - sourceLength, source.min);
    }
    else if (sourceLength > targetLength) {
        origin = easingLookup.progress(source.min, source.max - targetLength, target.min);
    }
...
function updateAxisDelta(delta, source, target, origin) {
    ...
    delta.originPoint = easingLookup.mix(source.min, source.max, delta.origin);
...
function convertTransitionToAnimationOptions(_a) {
    var yoyo = _a.yoyo, loop = _a.loop, flip = _a.flip, ease = _a.ease, transition = tslib.__rest(_a, ["yoyo", "loop", "flip", "ease"]);
...
function startPopmotionAnimate(transition, options) {
    hydrateKeyframes(options);
    return easingLookup.animate(tslib.__assign(tslib.__assign({}, options), convertTransitionToAnimationOptions(transition)));
}
function startPopmotionInertia(transition, options) {
    return easingLookup.inertia(tslib.__assign(tslib.__assign({}, options), transition));
}
/**
 *
 */
    ...
    VisualElementAnimationControls.prototype.animateVariant = function (variantLabel, opts) {
        ...
        var getChildrenAnimations = this.children
            ? function (forwardDelay) {
                ...
                return _this.animateChildren(variantLabel, delayChildren + forwardDelay, transition.staggerChildren, transition.staggerDirection, priority);
            }
            : function () { return Promise.resolve(); };
        /**
...
var PanSession = /** @class */ (function () {
    function PanSession(event, handlers, _a) {
        ...
        this.updatePoint = function () {
            ...
            var isDistancePastThreshold = easingLookup.distance(info.offset, { x: 0, y: 0 }) >= 3;
...
function applyConstraints(point, _a, elastic) {
    ...
    if (min !== undefined && point < min) {
        ...
        point = elastic ? easingLookup.mix(min, point, elastic) : Math.max(point, min);
    }
    else if (max !== undefined && point > max) {
        // If we have a max point defined, and this is outside of that, constrain
        point = elastic ? easingLookup.mix(max, point, elastic) : Math.min(point, max);
    }
    return point;
}
/**
...
function calcPositionFromProgress(axis, constraints, progress) {
    ...
    var min = easingLookup.mix(constraints.min, constraints.max - axisLength, progress);
        ...
        var onStart = function (event, info) {
            ...
            eachAxis(function (axis) {
                ...
                _this.cursorProgress[axis] = cursorProgress
                    ? cursorProgress[axis]
                    : easingLookup.progress(min, max, point[axis]);
                /**
...
function tweenAxis(target, prev, next, p) {
    target.min = easingLookup.mix(prev.min, next.min, p);
    target.max = easingLookup.mix(prev.max, next.max, p);
}
...
var Animate = /** @class */ (function (_super) {
    ...
    Animate.prototype.createCrossfadeAnimation = function (crossfadeOpacity) {
        ...
        return function (p) {
            opacity.set(easeCrossfadeIn(easingLookup.mix(0, 1, p)));
            crossfadeOpacity.set(easeCrossfadeOut(easingLookup.mix(1, 0, p)));
        };
    };
...
function compress(min, max, easing) {
    return function (p) {
        ...
        if (p > max)
            return 1;
        return easing(easingLookup.progress(min, max, p));
    };
}
...
function transform() {
    ...
    var interpolator = easingLookup.interpolate(inputRange, outputRange, tslib.__assign({ mixer: getMixer(outputRange[0]) }, options));
...
function useSpring(source, config) {
    ...
    React.useMemo(function () {
        return value.attach(function (v, set) {
            ...
            activeSpringAnimation.current = easingLookup.animate(tslib.__assign(tslib.__assign({ from: value.get(), to: v, velocity: value.getVelocity() }, config), { onUpdate: set }));
            return value.get();
        });
...
function useCycle() {
    ...
    return [
        ...
        function (next) {
            index.current =
                typeof next !== "number"
                    ? easingLookup.wrap(0, items.length, index.current + 1)
                    : next;
